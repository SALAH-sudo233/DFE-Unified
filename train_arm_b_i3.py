...
if __name__ == '__main__':
    ...
    if args.resume:
        logger.info(f'ARM-B warm-start (weights only) from: {args.resume}')
        ckpt = torch.load(args.resume, map_location=args.device, weights_only=False)
        model.load_state_dict(ckpt['model'])
        # ARM-B: I3 vector-safe structure; adapt weights to zero-origin.
        model.set_science_vector_origin('zero')
        # Warm-start WEIGHTS ONLY: fresh optimizer/scheduler, iteration from 0.
        start_iter = 0
        logger.info('ARM-B: zero-origin injected; optimizer/scheduler reset; training %d iters.' % config.train.max_iters)
        del ckpt
