...
if __name__ == '__main__':
    ...
    for name, param in model.named_parameters():
        if (os.environ.get('FREEZE_MODE','A') == 'A' and ('encoder' in name or 'field' in name)) or (os.environ.get('FREEZE_MODE','A') == 'B' and 'encoder' in name):
            param.requires_grad = False
            frozen += param.numel()
    logger.info('Conservative FT: mode=%s baseline=%s frozen_encoder_params=%d trainable_params=%d', os.environ.get('FREEZE_MODE','A'), ckpt_path, frozen, sum(p.numel() for p in model.parameters() if p.requires_grad))
