...
if __name__ == '__main__':
    ...
    ckpt_path = os.environ['CONSERVATIVE_CKPT']
    ckpt = torch.load(ckpt_path, map_location=args.device, weights_only=False)
    model.load_state_dict(ckpt['model'], strict=True)
    del ckpt
    torch.cuda.empty_cache()
    frozen = 0
    mode = os.environ.get('FREEZE_MODE', 'A')
    freeze_prefixes = {
        'A': ('protein_atom_emb.', 'ligand_atom_emb.', 'encoder.', 'df_module.', 'df_proj.', 'field.'),
        'B': ('protein_atom_emb.', 'ligand_atom_emb.', 'encoder.'),
    }
    if mode not in freeze_prefixes:
        raise ValueError(f'FREEZE_MODE must be A or B, got {mode!r}')
    frozen_names = []
    for name, param in model.named_parameters():
        if name.startswith(freeze_prefixes[mode]):
    ...
    if mode == 'A' and any(name.startswith(('df_module.', 'df_proj.', 'field.')) for name in trainable_names):
        raise AssertionError('A leaves DF/field parameters trainable')
    if mode == 'B' and any(name.startswith(('encoder.', 'protein_atom_emb.', 'ligand_atom_emb.')) for name in trainable_names):
        raise AssertionError('B leaves encoder/input embedding parameters trainable')
    logger.info('Conservative FT: mode=%s baseline=%s frozen_params=%d trainable_params=%d', mode, ckpt_path, frozen, sum(p.numel() for p in model.parameters() if p.requires_grad))
    logger.info('Frozen prefixes=%s trainable_prefixes=%s', freeze_prefixes[mode], sorted(set(n.split('.')[0] for n in trainable_names)))

    # Optimizer and scheduler
    optimizer = torch.optim.Adam([p for p in model.parameters() if p.requires_grad], lr=config.train.optimizer.lr, weight_decay=config.train.optimizer.weight_decay, betas=(config.train.optimizer.beta1, config.train.optimizer.beta2))
