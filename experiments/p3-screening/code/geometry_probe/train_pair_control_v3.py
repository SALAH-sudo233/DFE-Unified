...
EXP=Path('/workspace/ayb/experiments/dfe-unified-p3'); OUT=EXP/'geometry_probe'; RUN=OUT/'pair_control_run_v3'; RUN.mkdir(exist_ok=False)
...
elements=['H','C','N','O','F','P','S','CL','BR','I','B','SI','NA','MG','CA','FE','ZN','MN','CU','NI','CO','CD','SE','OTHER']; vocab=[f'{a}:{b}:{d}' for a in elements for b in elements for d in range(8)]+['OOV']; vi={k:i for i,k in enumerate(vocab)}
def enc(rs, source=None):
 ...
 for i,r in enumerate(rs):
  ...
  for k,v in rr['pair_hist'].items(): X[i,vi.get(k,vi['OOV'])]+=math.log1p(v)
  X[i]/=max(1.,X[i].sum())
 return X,y
...
def pear(a,b):
 a=np.asarray(a); b=np.asarray(b); a=a-a.mean(); b=b-b.mean(); return float((a*b).sum()/(np.linalg.norm(a)*np.linalg.norm(b)+1e-9))
class M(nn.Module):
