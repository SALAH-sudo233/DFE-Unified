# train_resume.py - Modified train.py with checkpoint resume support
# Usage: python train_resume.py --config configs/train_df.yml --logdir logs/train_df_2026_08_17__10_38_05 --resume logs/train_df_2026_08_17__10_38_05/checkpoints/270000.pt
import os
import shutil
import argparse
...
if __name__ == '__main__':
    ...
    # Logging - reuse existing log_dir if resuming
    if args.resume:
        log_dir = args.logdir  # Reuse existing directory
        start_iter = 0
    ...
    if args.resume:
        logger.info(f'ARM-CENTERED warm-start (weights only) from: {args.resume}')
        ckpt = torch.load(args.resume, map_location=args.device, weights_only=False)
        model.load_state_dict(ckpt['model'])
        # ARM-CENTERED: vector-safe structure with per-pocket centered origin.
        model.set_science_vector_origin('centered')
        # Warm-start WEIGHTS ONLY: fresh optimizer/scheduler, iteration from 0.
        start_iter = 0
        logger.info('ARM-CENTERED: centered-origin injected; optimizer/scheduler reset; training %d iters.' % config.train.max_iters)
        del ckpt
    ...
    def train(it):
        ...
        loss, loss_frontier, loss_pos, loss_cls, loss_edge, loss_real, loss_fake, loss_surf = model.get_loss(
            ...
            compose_batch = batch.compose_pos_batch,
        )
        if config.train.use_apex:
            ...
            loss.backward()
        orig_grad_norm = clip_grad_norm_(model.parameters(), config.train.max_grad_norm, error_if_nonfinite=True)
        optimizer.step()
    ...
    def validate(it):
        sum_loss, sum_n = np.zeros(5 + 2 + 1), 0
        with torch.no_grad():
            model.eval()
