...
- [ ] Add `dfe/science/model_precision.py` with strict `float32`/`float64`
  normalization and torch dtype mapping.
- [ ] Add RED tests for helper behavior, CLI exposure/defaults, report dtype,
  strict-load ordering, and repository source authorities.
- [ ] Thread `--model-dtype` through both SCI-1 and shared SE(3) audit runners.
...
- [ ] Run focused RED/GREEN tests, then local science/full repository gates,
